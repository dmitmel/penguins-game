...
void interactive_placement(Board* board, GameState* gameState){
  ...
  update_game_state_display(board, gameState);
  ...
  while (penguins_to_place > 0) {
    ...
    handle_placement_input(&target, &board, &(gameState->players[current_player]), gameState->playerCount);
  ...
  print_end_placement_phase(&board, gameState);
}